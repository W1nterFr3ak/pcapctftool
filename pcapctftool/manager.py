...
from pcapctftool import logger, extract, utils
from pcapctftool.session import SessionsManager, Session, stop_managed_sessions, SessionException
from pcapctftool.parsers import parsers, ntlmssp

stt = []
class MalformedPacketException(Exception):
    """
    This exception is raised when a malformed packet has been detected, therefore it cannot be analysed,
    or at least not by pcapctftool.
    """
    pass
...
def clean_before_exit():
    """
    Makes sure resources are closed properly before exiting pcapctftool.
    """
    ...
    try:
        for packet in packets_input:
            ...
            try:
                ...
                traceback.print_exc()
                logger.error("An exception occurred but pcapctftool will keep going.")
...
def process_pcap(filename: str, must_inspect_strings=False, tshark_filter=None, debug=False,
    ...
    with pyshark.FileCapture(filename, display_filter=tshark_filter, decode_as=decode_as, debug=debug) as pcap:
        ...
        if remaining_credentials:
            logger.info("Interesting things have been found but the pcapctftool wasn't able validate them: ")
            ...
            for session, remaining in remaining_credentials:
                if remaining and remaining not in printed_rem:
                    logger.info(session, str(remaining))
                    printed_rem.append(remaining)
